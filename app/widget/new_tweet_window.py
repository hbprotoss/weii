...
class NewTweetWindow(QDialog):
    ...
    def setupUI(self):
        ...
        for account in account_manager.getAllAccount():
            account_button = account_group.AccountButton(account)
            self.account_bar.addWidget(account_button)
            if account.if_send:
                self.selectAccount(account)
            # Receive signal
            self.connect(account_button, account_group.SIGNAL_SELECTED, self.selectAccount)
            self.connect(account_button, account_group.SIGNAL_UNSELECTED, self.unselectAccount)
