...
class TweetWidget(QWidget):
    ...
    def setupUI(self):
        ...
        if('retweeted_status' in self.tweet):
            ...
            h4 = QHBoxLayout()
            v3.addLayout(h4)
            str_time = time.strftime(self.time_format, time.localtime(int(retweet['created_at'])))
        ...
        h2 = QHBoxLayout()
        v2.addLayout(h2)
        str_time = time.strftime(self.time_format, time.localtime(int(self.tweet['created_at'])))
